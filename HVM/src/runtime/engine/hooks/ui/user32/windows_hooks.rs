...
impl VirtualExecutionEngine {
    ...
    fn user32_invoke_hook_callback(
        ...
        if hook.callback == 0 || !self.core.arch.is_x86() {
            return Ok(0);
        }
    ...
    pub(super) fn user32_register_hook(
        ...
        self.ui.user32_state.hooks.insert(
            handle,
            User32HookRecord {
                handle,
                hook_type,
        ...
        if hook_type == WH_MSGFILTER {
            ...
            Self::user32_queue_message(
                &mut self.ui.user32_state.pending_hook_messages,
                keyboard_seed,
            );
        }
    ...
    pub(super) fn user32_unhook(&mut self, handle: u32) -> bool {
        let removed = self.ui.user32_state.hooks.remove(&handle).is_some();
    ...
    pub(super) fn user32_pump_pending_hook_messages(
        ...
        for _ in 0..max_messages.max(1) {
            ...
            for handle in hook_handles {
                let Some(hook) = self.ui.user32_state.hooks.get(&handle).cloned() else {
                    continue;
                };
                let callback_result = self
                    .user32_invoke_hook_callback(&hook, &message)
                    .unwrap_or(0);
                self.ui.user32_state.hook_callback_dispatches = self
                    .ui
                    .user32_state
                    .hook_callback_dispatches
                    .saturating_add(1);
                if let Some(live_hook) = self.ui.user32_state.hooks.get_mut(&handle) {
