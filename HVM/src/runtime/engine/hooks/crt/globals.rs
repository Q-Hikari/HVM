...
impl VirtualExecutionEngine {
    pub(super) fn ensure_msvcrt_globals(&mut self) -> Result<u64, VmError> {
        if let Some(base) = self.crt.msvcrt_globals_base {
            return Ok(base);
        }
        let base =
            self.core
        ...
        self.write_pointer_value(
            base + MSVCRT_ONEXIT_TABLE_OFFSET + self.core.arch.pointer_size as u64,
            0,
        )?;
        self.write_pointer_value(
            base + MSVCRT_ONEXIT_TABLE_OFFSET + (self.core.arch.pointer_size as u64 * 2),
            0,
        )?;
        self.core
            .modules
        ...
        self.crt.msvcrt_globals_base = Some(base);
        Ok(base)
    }
    ...
    pub(super) fn ensure_msvcrt_acmdln_cell(&mut self) -> Result<u64, VmError> {
        ...
        let command_line = self.core.process_env.layout().command_line_ansi_buffer;
